...
const Navbar = () => {
  ...
  const changeLanguage = (url: string, newLang: string) => {
    ...
    if (url == "https://www.sirionsrl.com/whistleblowing") {
      ...
      if (urlParts[3] && urlParts[3].length === 2) {
        // Check if the part is a language code
  ...
  return (
    <>
      <nav
        className={`fixed top-0 w-full z-10 transition-colors duration-300 bg-customLightGrey ${navbarClass}`}
      >
          ...
          <div className="flex items-center justify-between h-20">
            ...
            <div className="hidden md:flex">
              <div className="ml-10 flex items-baseline space-x-4">
                <div
                  className="relative"
                  onMouseEnter={toggleMenuProd}
                  onMouseLeave={toggleMenuProd}
                >
                  ...
                  {isOpenProd && (
                    <div className="bg-customWhite absolute z-10 left-0 w-40 bg-white divide-y divide-gray-100  shadow-lg">
                      <Link
                        href="/"
                        className="text-customBlack block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
                      >
                        Intro
                      </Link>
                      <Link
                ...
                <div
                  className="relative"
                  onMouseEnter={toggleMenuKH}
                  onMouseLeave={toggleMenuKH}
                >
                  <button className="text-customBlack  hover:text-gray-300 px-3 py-2 rounded-md text-sm font-medium focus:outline-none flex items-center">
                    Prodotti
                    <ArrowDownIcon />
                ...
                <Link
                  href="/technology"
                  className="text-customBlack  hover:text-gray-300 px-3 py-2 rounded-md text-sm font-medium"
                >
                  Tecnologia
                </Link>
            ...
            <div className="md:hidden flex items-center">
              <button
                onClick={toggleMenu}
                className="text-customBlack px-3 py-2 rounded-md text-3xl font-medium focus:outline-none"
              >
      ...
      {isOpen && (
        <div
          className="fixed top-0 left-0 w-full h-full bg-customBlue z-50"
          onClick={toggleMenu}
        >
          <div className="flex justify-between items-center px-4 py-2 bg-customBlue">
            <button
              onClick={toggleMenu}
              className="text-customWhite hover:text-gray-300 focus:outline-none text-3xl"
            >
              <IconClose />
            </button>
            <div
          ...
          <div className="flex flex-col items-center justify-evenly flex-grow h-full">
            <Link
              href="/"
              className="text-customWhite hover:text-gray-300 px-3 py-2 rounded-md text-xl font-medium"
            >
              Home
            </Link>
            <Link
